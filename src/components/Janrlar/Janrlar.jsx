...
const Janrlar = () => {
    ...
    const fetchData = () => {
        try{
            fetch(`https://biryuzikki.uz/api/v1/general?devan_id=${selectedCard}`).then(res => res.json().then(data => {
    ...
    return (
        <div className={css.janrlarWrapper}>
            ...
            <ul className={css.janrlarNameList}>
                <li className={`${css.janrlarNameItem} ${selectedGenre.name === selectedGenre.name ? css.active : ''}`} onClick={() => setSecondaryData("main")}>
